...
class Command(BaseCommand):
    ...
    def write_safe(self, style_func, text):
        try:
            ...
            try:
                self.stdout.write(style_func(cleaned))
            except Exception:
                pass
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(image_path):
            self.write_safe(self.style.ERROR, f"❌ 圖片路徑不存在: {image_path}")
            return

        self.write_safe(self.style.WARNING, "正在註冊 Rich Menu...")
            ...
            if list_res.status_code == 200:
                ...
                for menu in existing_menus:
                    if menu.get("name") == "CSI Portal 三區選單 V4":
                        ...
                        if match:
                            self.write_safe(self.style.SUCCESS, f"ℹ️ 已存在完全一致的選單 (ID: {menu['richMenuId']})，無需重複註冊。")
                            # 確保它是預設選單
                            ...
                            # 結構有更新，刪除該舊選單
                            self.write_safe(self.style.WARNING, f"🗑️ 檢測到選單結構有更新，正在刪除舊選單 (ID: {menu['richMenuId']})...")
                            delete_url = f"https://api.line.me/v2/bot/richmenu/{menu['richMenuId']}"
                            requests.delete(delete_url, headers=list_headers)
        except Exception as e:
            self.write_safe(self.style.WARNING, f"⚠️ 查詢選單列表發生錯誤: {e}")
        ...
        try:
            ...
            if res.status_code != 200:
                self.write_safe(self.style.ERROR, f"❌ LINE Rich Menu 建立失敗: {res.text}")
                return

            rich_menu_id = res.json()["richMenuId"]
            self.write_safe(self.style.SUCCESS, f"✅ Rich Menu 建立成功，ID: {rich_menu_id}")

            # 2. 上傳背景圖片
            self.write_safe(self.style.WARNING, "正在上傳選單圖片...")
            ...
            if upload_res.status_code != 200:
                self.write_safe(self.style.ERROR, f"❌ 圖片上傳失敗: {upload_res.text}")
                return

            self.write_safe(self.style.SUCCESS, "✅ 背景圖片上傳成功！")

            # 3. 設為該帳號的預設 Rich Menu
            self.write_safe(self.style.WARNING, "正在套用預設選單...")
            ...
            if default_res.status_code != 200:
                self.write_safe(self.style.ERROR, f"❌ 設定預設選單失敗: {default_res.text}")
                return

            self.write_safe(self.style.SUCCESS, "🎉 成功！三區圖文選單已生效並套用為預設。")

        except Exception as e:
            self.write_safe(self.style.ERROR, f"❌ 發生異常錯誤: {e}")
