...
def generate_ai_content(file_path, text_content, existing_summary=None, existing_questions=None, is_exam_paper=False):
    ...
    if is_exam_paper:
        prompt += """
        這是一份「歷屆考題/模擬試題」的完整文件。
        任務：仔細閱讀資料來源，提取出裡面的「選擇題」或根據內容出題。
        - 無視原文件題號重置：這份文件包含多個章節（例如 L11101、L11102 等），且每個章節的題號都會從 1 重新開始。請你「完全無視」原本的題號！
        - 強制全局連續編號：請將所有萃取出來的題目進行「全局連續編號」。例如第一章是 1~10 題，第二章的第一題必須被編號為第 11 題，以此類推，直到整份文件的最後一題。
